...
`starter-decks/*.json` are batches of cards delivered as **backup files**, so the owner imports them
with Backup → Open a backup → **Merge**. There is no other way in: browser storage cannot be
written from outside the browser.

They are generated by `npx tsx scripts/build-decks.ts` from the data in `scripts/decks/`. Rules for
extending them:

- **Card ids are fixed and must never change for a published batch.** They are what makes importing
  idempotent — merging the same file twice adds nothing. Changing an id would silently duplicate a
  card on the owner's device.
- **Colour positions are found by naming the phrase, never hand-counted.** A phrase that is missing
  or ambiguous throws, so a typo fails loudly rather than producing a card that looks right and
  teaches nothing.
- Categories in the file are dated 2020 so a renamed category on the device always wins the merge.
- Colour meanings carry no timestamp, so the owner's labels always win.
- Everything lands in `category-dont-know`.
- Keep colour **sparse**: gender on nouns, stress only where a written accent already marks it,
  and orange only for genuine traps. Adjectives and adverbs carry none.
- After regenerating, check `git diff` shows no change to already-published batches.
